...
const createCollection = async (req, res) => {
  ...
  const collectionData = {
    ...
    bgColor: req.body.bgColor || '#1f2a48', // gp default if not provided
    borderColor: req.body.borderColor || '#3b73ff', // go default if not provided
...
const editCollection = async (req, res) => {
  try {
    const {
      id, name, description, bgColor, borderColor,
    } = req.body;
    ...
    const updated = await Collection.updateOne(
      ...
      {
        $set: {
          name, description, bgColor, borderColor,
        },
      },
    );
